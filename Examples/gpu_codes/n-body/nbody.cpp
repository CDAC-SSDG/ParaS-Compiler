...
{
  arithmetic_type delta = paras::sycl::fabs(mirror_pos - position);
...
using local_accessor =
  paras::sycl::accessor<T,Dim,
                 paras::sycl::access::mode::read_write,
                 paras::sycl::access::target::local>;
...
int main()
{
  ...
  auto particles_buffer =
      paras::sycl::buffer<particle_type, 1>{particles.data(), particles.size()};
  auto velocities_buffer =
      paras::sycl::buffer<vector_type, 1>{velocities.data(), velocities.size()};
  auto forces_buffer =
      paras::sycl::buffer<vector_type, 1>{paras::sycl::range<1>{particles.size()}};

  paras::sycl::default_selector selector;
  paras::sycl::queue q{selector};

  auto execution_range = paras::sycl::nd_range<1>{
	  paras::sycl::range<1>{((num_particles + local_size - 1) / local_size) * local_size},
      paras::sycl::range<1>{local_size}
  };
  ...
  for(std::size_t t = 0; t < num_timesteps; ++t)
  {
    ...
    q.submit([&](paras::sycl::handler& cgh){
      auto particles_access =
          particles_buffer.get_access<paras::sycl::access::mode::read>(cgh);
      auto forces_access =
          forces_buffer.get_access<paras::sycl::access::mode::discard_write>(cgh);
        ...
        for(size_t offset = 0; offset < num_particles; offset += local_size)
        {
          ...
          for(int i = 0; i < local_size; ++i)
          {
            ...
            const arithmetic_type r_inv =
                paras::sycl::rsqrt(R.x()*R.x() + R.y()*R.y() + R.z()*R.z()
    ...
    q.submit([&](paras::sycl::handler& cgh){
      auto particles_access =
          particles_buffer.get_access<paras::sycl::access::mode::read_write>(cgh);
      auto velocities_access =
          velocities_buffer.get_access<paras::sycl::access::mode::read_write>(cgh);
      auto forces_access =
          forces_buffer.get_access<paras::sycl::access::mode::read>(cgh);
    ...
    if(t % iterations_per_output == 0)
    {
      ...
      auto particle_positions =
          particles_buffer.get_access<paras::sycl::access::mode::read>();
